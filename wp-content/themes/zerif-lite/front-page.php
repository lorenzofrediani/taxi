						...
						<div class="row">
							...
							<form role="form" method="POST"  onSubmit="this.scrollPosition.value=(document.body.scrollTop || document.documentElement.scrollTop)" class="contact-form">
								...
								if( !empty($zerif_contactus_button_label) ):
									echo '<button id="pirate-forms-contact-submit"  class="btn btn-primary custom-button red-btn" type="submit" data-scrollreveal="enter left after 0s over 1s">'.$zerif_contactus_button_label.'</button>';
								elseif ( is_customize_preview() ):
									echo '<button id="pirate-forms-contact-submit"  class="btn btn-primary custom-button red-btn zerif_hidden_if_not_customizer" type="submit" data-scrollreveal="enter left after 0s over 1s"></button>';
								endif;
								?>
    <script>
    document.getElementById("pirate-forms-contact-submit").addEventListener("click", function(e){
	var check = document.getElementById("check_agree");
	if( !check.checked ){
	  alert("Condizioni della privacy non accettate");
	  e.preventDefault();
	}
});
</script>
                  <label style="color:white">
<input id="check_agree" value="" type="checkbox" />Accetto le <a href="http://www.iubenda.com/privacy-policy/7962218/legal" target="_blank">condizioni della privacy</a></label>
